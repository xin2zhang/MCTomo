...
c-----
     ...
     &                       mode,igr,kmax,t,dphase,cp,cg,ierr)
        ...
        integer NP
        parameter (NP=60)
        ! increment of phase velocity
        real(kind=8) :: dphase
        double precision :: cp(kmax)
        ! err flag
        integer ierr
...
c----- 
        ...
        double precision t(NP),c(NP),cb(NP),cg(kmax)
...
c -----         calculate group velocity and output phase and group velocities
				...
				cg(k) = gvel
                cp(k) = cc0
                if(gvel<0 .or. c(k)==0)then
                    ierr = 1
                    !write(*,*) 'gvel is smaller than zero: ', gvel
                endif
...
c   mod    e, just indicate (itst=0) that it has not been found, but
c   fil    l out file with dummy results to maintain format - note
c   eig    enfunctions will not be found for these values. The subroutine
c   'am    at' in 'surf' will worry about this in building up the
c   inp    ut file for 'surfinv'
c-----
...
! multimodes calculation
       subroutine surfdisp_mmodes(thkm,vpm,vsm,rhom,nlayer,iflsph,iwave,
     &                       mode,igr,kmax,t,dphase,cp,cg,ierr)
...
c----- 
        ...
        double precision twopi,one,onea
        double precision cc,c1,clow,cm,dc,t1
        double precision :: cp(kmax,mode)
        double precision t(NP),c(NP),cb(NP),cg(kmax,mode)
        real*4 d(NL),a(NL),b(NL),rho(NL),rtp(NL),dtp(NL),btp(NL)		
c        common/modl/ d,a,b,rho,rtp,dtp,btp		
c        common/para/ mmax,llw,twopi
        integer*4 iverb(2)
