...
module m_hmc
    ...
    use m_likelihood,  only : update_lgP_grads, update_lgP_grads_approx
    ...
    real(kind=ii10), parameter :: threshold = -1.0
...
contains
    ...
    subroutine hmc_step(target_data, RTI, like_set, hmc_set, qvals_in, lgP,accept) 
        implicit none
        type(T_DATA), intent(in)                    :: target_data
        ...
        type(T_LIKE_BASE), intent(inout)                 :: lgP
        ...
        do i = 1, hmc_set%n_leapfrogs
            ...
            ! debug
            !write(*,*) 'Velocity grads', lgP%grads(1:RTI%ncells)
            !write(*,*) 'Velocity', qvals(1:RTI%ncells)
            !write(*,*) 'Velocity', RTI%parameters(1,1:RTI%ncells)
            !write(*,*) 'Velocity grads', grad(1:RTI%ncells)
            ...
            !call log_msg('distance: '// rtoa(norm2(rti%parameters(1,1:rti%ncells)-qvals_in)))
    ...
    subroutine update_model(qvals, hmc_set, rti, like_set)
        ...
        ! test for m/s
        !rti%parameters(1,:) = rti%parameters(1,:)/1000
        !rti%parameters(2,:) = rti%parameters(2,:)/1000
