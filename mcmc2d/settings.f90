...
module m_settings
    ...
    type T_MCMC_SET
	    ...
	    real(kind=ii10) :: dn0_min, dn0_max
	    real(kind=ii10) :: dn1_min, dn1_max
	    ...
	    real(kind=ii10) :: sigma_dn0, sigma_dn1
...
contains
    subroutine grid_setup(grid)
        ...
        if(grid%scaling>0)then
            grid%ymin = grid%ymin*grid%scaling
            grid%ymax = grid%ymax*grid%scaling
        endif
    ...
    logical function out_bnd(coord, bnd)
	    ...
	    if( coord(1) < bnd(1) .or. coord(1) > bnd(2) &
            .or. coord(2) < bnd(3) .or. coord(2) > bnd(4)) then
	    	out_bnd = .true.
	    else
	    	out_bnd = .false.
	    endif
