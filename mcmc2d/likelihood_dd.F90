...
module m_body_likelihood
    ...
    public :: body_likelihood_grads, body_update_lgP_grads_approx
...
contains
    ...
    subroutine body_noise_likelihood(dat,RTI,settings,like)
        ...
        integer nrr
        integer nsrc, nrev
        integer i, j, k, ddindex, sumdsigma
        ...
        do j = 1, nsrc
               do k = 1, nrev
                ...
                if(dat%raystat(nrr,1,1) == 1) then
                    ...
                    like%like = like%like &
                        + ( like%phaseTime(k,j,1)-dat%ttime(nrr,1,1))**2/( 2*(like%sigma(nrr,1))**2 )
                    like%misfit = like%misfit & 
                        + ( like%phaseTime(k,j,1)-dat%ttime(nrr,1,1))**2/(like%sigma(nrr,1)**2)
                else
        ...
        ! add the differential time
        if(settings%datatype == 1)then
            ddindex=0
            do i = 1, nsrc
                do j = i+1, nsrc
                    ddindex = ddindex + 1  
                    do k = 1, nrev
                        like%dsigma(k,ddindex,1) = RTI%snoise0(dat%np+1)*(1-dat%dcc(k,ddindex,1)) + RTI%snoise1(dat%np+1)
                    enddo
                enddo
            enddo
        endif
        ...
        if(settings%datatype == 1)then
            ...
            do i = 1, nsrc
                do j = i+1, nsrc
                    ddindex = ddindex + 1  
                    do k = 1, nrev
                        if(abs(dat%dtime(ddindex,k,1))>eps)then
                            like%like= like%like &
                                + (like%dtime(k,ddindex,1)-dat%dtime(k,ddindex,1))**2/(2*(like%dsigma(k,ddindex,1))**2)
                            like%misfit= like%misfit &
                                + (like%dtime(k,ddindex,1)-dat%dtime(k,ddindex,1))**2/(like%dsigma(k,ddindex,1)**2)
                            sumdsigma = sumdsigma + log(like%dsigma(k,ddindex,1))
                        endif
                    enddo
                enddo
            enddo
            like%like = like%like + sumdsigma + dat%ndtime/2.0 * log(PI2)
        endif
        return
    ...
    subroutine body_update_lgP_grads_approx(dat,RTI,settings,like)
        implicit none
        type(T_DATA), intent(in)                    :: dat
        type(T_RUN_INFO), intent(inout)             :: RTI
        type(T_LIKE_SET), intent(in)                :: settings
    ...
    endsubroutine body_update_lgP_grads_approx
    ...
    subroutine body_likelihood(dat,RTI,settings,like)
        ...
        if(settings%sigdep /= 0)then
            do i = 1, dat%np
                like%srdist(:,i) = reshape(like%phaseTime(:,:,i),(/nsrc*nrev/))
                like%sigma(:,i) = RTI%snoise0(i)*like%srdist(:,i) + RTI%snoise1(i)
            enddo
        else
            like%sigma = dat%ttime(:,2,:)
        ...
        if(settings%datatype==1)then
            ...
            do i = 1, nsrc
                do j = i+1, nsrc
                    ddindex = ddindex + 1  
                    do k = 1, nrev
                        like%dtime(k,ddindex,1) = like%phaseTime(k,j,1) - like%phaseTime(k,i,1)
                        like%dsigma(k,ddindex,1) = RTI%snoise0(dat%np+1)*(1-dat%dcc(k,ddindex,1)) + RTI%snoise1(dat%np+1)
                    enddo
                enddo
            enddo
                ...
                do j = i+1, nsrc
                    ...
                    do k = 1, nrev
                        if(abs(dat%dtime(ddindex,k,1))>eps)then
                            ...
                            like%misfit= like%misfit &
                                + (like%dtime(k,ddindex,1)-dat%dtime(k,ddindex,1))**2/(like%dsigma(k,ddindex,1)**2)
                            like%unweighted_misfit= like%unweighted_misfit &
                                + (like%dtime(k,ddindex,1)-dat%dtime(k,ddindex,1))**2
                            sumdsigma = sumdsigma + log(like%dsigma(k,ddindex,1))
                        endif
                    enddo
                enddo
    ...
    subroutine derivative2(points,parameters,grid,rays,time_data,time_noise,time_out,grads)
        ...
        real(kind=ii10), dimension(2) :: raypoint
        ...
        do i = 1, size(rays)
            ...
            one_grads = 0
            raypoint(1) = rays(i)%points(1,1)
            raypoint(2) = rays(i)%points(2,1)*grid%scaling
            call kdtree2_n_nearest(tp=tree,qv=raypoint,nn=1,results=results)
            prev_id = results(1)%idx
            do j = 2, rays(i)%npoints
                raypoint(1) = rays(i)%points(1,j)
                raypoint(2) = rays(i)%points(2,j)*grid%scaling
                call kdtree2_n_nearest(tp=tree,qv=raypoint,nn=1,results=results)
    ...
    integer function sites_locate(tree,sites_id,grid,point)
        ...
        type(kdtree2_result), dimension(1) :: results
        real(kind=c_double), dimension(2) :: pt
        integer ix, iy
