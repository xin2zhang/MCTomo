...
contains
    ...
    subroutine read_temperatures(temperatures, ntemps)
	    ...
	    do i = 1, nsamples
            read(temp_unit,double_format(nd),iostat=stat) temperatures(:,i)
            if(stat/=0) exit
            ntemps = ntemps + 1
	    enddo
...
program sample
    ...
    do ichain = 1, nchains
        ...
        do
            ...
            if(mcmc_set%tempering==1)then
                call read_temperatures(temperatures,nsamples2)
            ...
            do while(iter<nsamples)
                ...
                if(ncount >= burn_in(ichain) .and. mod(ncount-burn_in(ichain), mcmc_set%thin) == 0 &
                    ...
                    if(ncount-burn_in(ichain) == 0)then
                        ...
                        if(ichain==1) norm = -1.0*samples(iter)%like
