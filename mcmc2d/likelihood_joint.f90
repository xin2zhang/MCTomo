...
module m_likelihood
    ...
    use m_body_likelihood, only : body_likelihood, body_likelihood_grads, body_noise_likelihood
...
contains
    ...
    subroutine likelihood(dat,RTI,like_set,like)
        implicit none
        type(T_DATA), dimension(:), intent(in)      :: dat
        ...
        case (0,1)
            call body_likelihood(dat(1),RTI,like_set,like%likelihoods(1))
        ...
        case (2)
            ...
            call surf_likelihood(dat(1),RTI,like_set,like%likelihoods(1))
            like%like = like%likelihoods(1)%like
            like%misfit = like%likelihoods(1)%misfit
            like%unweighted_misfit = like%likelihoods(1)%unweighted_misfit
        ...
        case (3)
            call body_likelihood(dat(1),RTI,like_set,like%likelihoods(1))
            call surf_likelihood(dat(2),RTI,like_set,like%likelihoods(2))
    ...
    !subroutine location_likelihood(dat,RTI,like_set,like)
    !    implicit none
    !    type(T_DATA), dimension(:), intent(in)      :: dat
    !    type(T_RUN_INFO), intent(inout)             :: RTI
    !    type(T_LIKE_SET), intent(in)                :: like_set
    ...
    !    call body_location_likelihood(dat(1),RTI,like_set,like%likelihoods(1))
