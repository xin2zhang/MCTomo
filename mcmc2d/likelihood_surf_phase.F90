...
contains
    ...
    subroutine surf_noise_likelihood(dat,RTI,like)
        ...
        do i = 1, dat%np
            do j = 1, dat%nrays
                if(dat%raystat(j,1,i) == 1) then
                    like%like = like%like + ( like%phaseTime(1,j,i)-dat%ttime(j,1,i)&
                        )**2/( 2*(like%sigma(j,i))**2 )
                    like%misfit = like%misfit + ( like%phaseTime(1,j,i)-dat%ttime(j,1,i)&
                        )**2/(like%sigma(j,i)**2)
                else
                    like%sigma(j,i) = 1.0
                endif
    ...
    subroutine surf_likelihood(dat,RTI,settings,like)
        ...
        if(settings%phaseGroup==0)then
            like%phaseTime(1,:,:)=transpose(pvel)
        else
            like%phaseTime(1,:,:)=transpose(gvel)
        endif
        ...
        if(settings%sigdep /= 0)then
            do i = 1, dat%np
                do j = 1, dat%nrays
                    if(dat%raystat(j,1,i) == 1) then
                        like%sigma(j,i) = RTI%snoise0(i)*like%srdist(j,i) + RTI%snoise1(i) 
                    else
                        like%sigma(j,i) = 1.0
                    endif
                enddo
        ...
        do i = 1, dat%np
            do j = 1, dat%nrays 
                if(dat%raystat(j,1,i) == 1) then
                    ...
                    like%unweighted_misfit = like%unweighted_misfit + ( like%phaseTime(1,j,i)-dat%ttime(j,1,i)&
                        )**2
                else
                    like%sigma(j,i) = 1.0
                endif
