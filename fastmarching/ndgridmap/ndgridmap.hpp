/*! \class nDGridMap
    ...
    are no longer assumed to be cubic, that is, the size (leaf size) of each cell can be 
    different in different dimension.
...
template <class T, size_t ndims> class nDGridMap {
    ...
    (std::ostream & os, nDGridMap<T,ndims> & g) {
        ...
        for (unsigned int i = 0; i < ndims; ++i)
            os << "\t\t" << "d" << i << "\tleafsize: " << g.leafsize_[i] << std::endl;
    ...
    public:
      ...
      /** @param dimsize constains the size of each dimension.
        ...
        nDGridMap
        (const std::array<unsigned int, ndims> & dimsize, const std::array<double, ndims>& leafsize) :
        clean_(true) {
            leafsize_ = leafsize;
            resize(dimsize);
        }
        ...
        inline std::array<double, ndims> getLeafSize() const { return leafsize_; }

        inline void setLeafSize(const std::array<double, ndims>& leafsize) { leafsize_ = leafsize; }
        ...
        /* Computes the surrounded nodes of a double point */
        unsigned int getNeighbors
        ( const std::array<double,ndims> & point, std::array<unsigned int, 1 << ndims> & neighs){
            std::array<unsigned int,ndims> coord;
            getPointCoord( point, coord);

            for (int v=0; v < (1 << ndims); v++) {
                std::array<unsigned int, ndims> v_coord;
                std::array<unsigned int, ndims> v_index;
                for (int dim=0; dim < ndims; dim++) {
                    v_index[dim] = ( v >> (ndims-dim-1) ) & 1;
                    v_coord[dim] = coord[dim] + v_index[dim];
                    if(v_coord[dim] < 0) v_coord[dim]=0;
                    if(v_coord[dim] >= dimsize_[dim]) v_coord[dim] = dimsize_[dim]-1;
                }
                coord2idx(v_coord,neighs[v]);
            }

            return 1<<ndims;
        }

        /* Computes the int coord of a double point */
        void getPointCoord(const std::array<double, ndims> & point, std::array<unsigned int,ndims> & index){

            for (int dim=0; dim < ndims; dim++) {
                 int c = int( point[dim]/leafsize_[dim] );
                 if( c < 0) {
                     c = 0;
                 }else if ( c >= dimsize_[dim] ) {
                     c = dimsize_[dim]-1;
                 }
                 index[dim] = c;
            }
        ...
        /* Computes the indices of 2 neighs in each direction for a specific dimension of the cell idx */
        void getNeighborsInDim
        (unsigned int idx, std::array<std::pair<unsigned int,unsigned int>, 2>& neighs, unsigned int dim) {
            // Candidate neighbors in dimension.
            n_neighs = 0;
            unsigned int c1a,c1b,c2a,c2b;
            if (dim == 0) {
                c1a = idx-1;
                c1b = c1a-1;
                c2a = idx+1;
            ...
            // Checking neighbor 1: is in the same n-dimensional slice (row, plane, cube, etc)?
            if (!((c1a >= 0) && (c1a/d_[dim] == idx/d_[dim])))
                c1a = std::numeric_limits<unsigned int>::max();
            if (!((c1b >= 0) && (c1b/d_[dim] == idx/d_[dim])))
                c1b = std::numeric_limits<unsigned int>::max();
            neighs[n_neighs++] = std::make_pair(c1a,c1b);
            // Checking neighbor 2. Full check, not necessary: if >ncells_ then is in another slice.
            if (c2a>=ncells_ || c2a/d_[dim] != idx/d_[dim])
                c2a = std::numeric_limits<unsigned int>::max();
            if (c2b>=ncells_ || c2b/d_[dim] != idx/d_[dim])
                c2b = std::numeric_limits<unsigned int>::max();
            neighs[n_neighs++] = std::make_pair(c2a,c2b);
        }
       ...
       /** \brief Shows the coordinates from an index. */
        ...
        void getPoint( unsigned int index, std::array<double,ndims> & point){
            std::array<unsigned int, ndims> coord;
            idx2coord(index,coord);
            for ( int i=0; i<ndims; i++){
                point[i] = coord[i]*leafsize_[i];
            }
        }

        double getVel( const std::array<double,ndims> & point ) {

            std::array<unsigned int, ndims> point_coord;
            getPointCoord(point, point_coord);

            std::array<unsigned int, 1<<ndims > neighbors;

            int n = getNeighbors(point, neighbors);

            double arrivaltime = 0;
            for ( int i = 0; i < n; i++ ) {
                std::array<unsigned int,ndims> coord;
                idx2coord(neighbors[i],coord);
                std::array<unsigned int,ndims> d_index;
                for ( int j = 0; j< ndims; j++){
                    d_index[j] = coord[j] - point_coord[j];
                }
                double w = 1.0;
                for( int j = 0; j < ndims; j++) {
                    double s = point[j] - leafsize_[j]*point_coord[j];
                    double wj = fabs( d_index[j]*leafsize_[j]-leafsize_[j]+s);
                    w = w*wj/leafsize_[j];
                }
                arrivaltime = arrivaltime + w*cells_[neighbors[i]].getVelocity();
            }

            return arrivaltime;
        } 

        double getTime( const std::array<double,ndims> & point ) {
            ...
            for ( int i = 0; i < n; i++ ) {
                ...
                arrivaltime = arrivaltime + w*cells_[neighbors[i]].getArrivalTime();
            }
    ...
    private:
        ...
        std::array<double, ndims> leafsize_;
