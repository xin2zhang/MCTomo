...
template < class grid_t > class DDQM : public EikonalSolver<grid_t> {
    ...
    public:
        ...
        (grid_t * g) {
            ...
            std::array<double, grid_t::getNDims()> leafsize ;
            leafsize = grid_->getLeafSize();
            thStep_  = 1.5 * (*std::max_element(leafsize.begin(),leafsize.end())) / grid_->getAvgSpeed();
            threshold_ = thStep_;
        }
        ...
        () {
            ...
            std::array<double, grid_t::getNDims()> leafsize ;
            leafsize = grid_->getLeafSize();
            double avgSpeed = 1.0/(grid_->getAvgSpeed()*(*std::max_element(leafsize.begin(),leafsize.end())));
