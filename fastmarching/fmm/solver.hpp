...
class Solver {
    ...
    public:
        ...
        (const std::vector<unsigned int> & init_points, unsigned int goal_idx) {
            ...
            init_points_times_.assign(init_points_.size(),0.0);
            goal_idx_ = goal_idx;
        }

        /** \brief Sets the initial and goal points by the indices of the grid. */
        virtual void setInitialPointsAndTimes
        (const std::vector<unsigned int> & init_points, const std::vector<double> & init_times) {
            init_points_ = init_points;
            init_points_times_ = init_times;
            goal_idx_ = -1;
        }

        virtual void setInitialPoints
        (const std::array<double, grid_t::getNDims()> & init_point) {

            std::array<unsigned int, (1 << grid_t::getNDims())> neighbor_index;

            double v = grid_->getVel(init_point);

            grid_->getNeighbors(init_point,neighbor_index);

            std::vector<unsigned int> init_points(neighbor_index.begin(),neighbor_index.end());
            std::vector<double> init_times;
            for( int i=0; i < (1<<grid_t::getNDims()); i++){
                 std::array<double,grid_t::getNDims()> current_point;
                 grid_->getPoint(neighbor_index[i],current_point);
                 double distance = calDistance(current_point,init_point);
                 double arrival = distance*2/(v+grid_->getCell(neighbor_index[i]).getVelocity());
                 init_times.push_back(arrival);
            }

            setInitialPointsAndTimes(init_points, init_times);
        }

        double calDistance
        (const std::array<double, grid_t::getNDims()> &p1, const std::array<double, grid_t::getNDims()> &p2 )
        {
            int ndims = grid_t::getNDims();
            double distance = 0;

            for (int i =0; i<ndims;i++){
                double xi = (p1[i] - p2[i])*(p1[i]-p2[i]);
                distance = distance + xi;
            }
    ...
    protected:
        ...
        /** Initial time */
        std::vector<double> init_points_times_;
