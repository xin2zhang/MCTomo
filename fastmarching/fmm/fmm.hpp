...
template < class grid_t, class heap_t = FMDaryHeap<FMCell> >  class FMM : public EikonalSolver<grid_t> {
    ...
    public:
        FMM(HeurStrategy h = NOHEUR) : EikonalSolver<grid_t>("FMM"), heurStrategy_(h), precomputed_(false), highAccuracy_(true) {
            /// \todo automate the naming depending on the heap.
            //if (static_cast<FMFibHeap>(heap_t))
             //   name_ = "FMMFib";
        }

        FMM(const char * name, bool accuracy = true, HeurStrategy h = NOHEUR ) : EikonalSolver<grid_t>(name), highAccuracy_(accuracy), heurStrategy_(h), precomputed_(false) {}
        ...
        () {
            ...
            for (unsigned int &i: init_points_) { // For each initial point
                grid_->getCell(i).setArrivalTime(init_points_times_[k]);
                grid_->getCell(i).setState(FMState::NARROW);
                k = k + 1;
            ...
            while (!stopWavePropagation && !narrow_band_.empty()) {
                ...
                for (unsigned int s = 0; s < n_neighs; ++s) {
                    ...
                    if ((grid_->getCell(j).getState() == FMState::FROZEN) || grid_->getCell(j).isOccupied())
                        ...
                        double new_arrival_time = 0.0;
                        if(highAccuracy_)
                            new_arrival_time = highAccuracySolveEikonal(j);
                        else
                            new_arrival_time = solveEikonal(j);
        ...
        /* \brief Set the order of fast marching */
        void setHighAccuracy(const bool ha) {
            highAccuracy_ = ha;
        }
    ...
    protected:
        ...
        using EikonalSolver<grid_t>::init_points_times_;
        ...
        using EikonalSolver<grid_t>::highAccuracySolveEikonal;
        using EikonalSolver<grid_t>::solveEikonalInOne;
        using EikonalSolver<grid_t>::highAccuracySolveEikonalInOne;
    ...
    private:
        ...
        /* \brief Flag of second order fast marching (high accuracy). */
        bool                                            highAccuracy_;
